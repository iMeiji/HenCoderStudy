...
public class Practice04DispatchDrawLayout extends LinearLayout {
    private static final String TAG = "Practice04DispatchDrawL";
    Pattern pattern = new Pattern();

    {
        setWillNotDraw(false);// 确保执行 onDraw 方法
    }
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        Log.d(TAG, "onDraw: ");
        pattern.draw(canvas);// 绘制内容被子 View 覆盖掉
    }
    ...
    protected void dispatchDraw(Canvas canvas) {
        ...
        pattern.draw(canvas);//在子 View 绘制后再绘制
    }
